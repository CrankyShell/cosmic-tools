...
const ToolCard = ({ title, desc, icon, link, image }) => (
  <motion.div 
    ...
    className="bg-cosmic-card border border-white/10 rounded-xl overflow-hidden shadow-lg hover:shadow-purple-500/20 transition-all duration-300 flex flex-col"
  >
    <div className="h-40 bg-gray-900 relative">
        <img src={image} alt={title} className="w-full h-full object-cover opacity-80 hover:opacity-100 transition"/>
        <div className="absolute top-4 right-4 bg-black/50 p-2 rounded-full text-purple-400">
            {icon}
        </div>
    </div>
    <div className="p-6 flex flex-col flex-grow">
      ...
      <Link 
        to={link}
        className="w-full py-3 bg-gradient-to-r from-purple-700 to-indigo-800 text-white font-bold text-center rounded-lg hover:from-purple-600 hover:to-indigo-700 transition"
      >
        OPEN TOOL
      </Link>
    </div>
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <section className="text-center py-16">
        ...
        <p className="text-xl text-gray-300 max-w-2xl mx-auto mb-10 leading-relaxed">
          Welcome to <span className="text-purple-400 font-bold">Cosmic Tools</span>. 
          The ultimate privacy-focused ecosystem for the modern trader. 
          Journal your journey, analyze the noise, and manage your risk—all in one place.
        </p>
        
        <div className="flex justify-center gap-4">
            <Link to="/about" className="px-8 py-3 rounded-full border border-purple-500 text-purple-400 hover:bg-purple-500/10 transition">
                About The Project
            </Link>
